...
jQuery(function(){
	...
	// On form submit, get the details of the form and send them off
	$(document).on(
		{
			submit: (event) => {
				...
				for (let i = 0; i < inputs.length; i++) {
					...
					if (input.name === 'keyring-name') {
						keyringName = input.value;
					}
					else if (input.name === 'keyring-id') {
						keyringID = input.value;
					}
					else if (input.name === 'email-address') {
						email = input.value;
					}
					else if (input.name.includes('MERGE||')) {
						let mergeName = input.name.replace('MERGE||', '');
						let mergeValue = input.value;
